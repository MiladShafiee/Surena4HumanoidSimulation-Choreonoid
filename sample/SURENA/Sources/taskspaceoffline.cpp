...
YOffsetOfAnkletrajectory=0.00;//for compensating the clearance of the hip roll in experiment
    ...
    Delta=0.00;// Domain of pelvis movemevt in z direction
    NStride=5;
    DesiredVelocity=0.2;
    StepLength=0.15;
    ...
    if (time<=TStart){//left foot moves in first step
        ...
        if (tt<TDs){//first double support of cyclic walking
            ...
            if (tt<T_beta){
                ...
                pitch_al=outputP(0,0);//first compute pitch angle of foot then use this angle for height and horizental trajectory of foot during heel landing as following
